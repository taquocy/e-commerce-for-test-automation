...
class UpdateProductPage():
    def __init__(self, driver):
        self.driver = driver
        self.edit_button = (By.CSS_SELECTOR, ".chakra-button.css-ljmwpa:first-of-type")  
    def update_product_first(self):
        self.driver.find_element(*self.edit_button).click()
        # Xác định các phần tử trên trang login
        self.title_label = (By.XPATH, "//label[@for='field-:r1d:']")  # Label cho Title
        ...
        self.description_label = (By.XPATH, "//label[@for='field-:r1e:']")  # Label cho Description
        self.description_input = (By.XPATH, "//textarea[@name='description']")  # Textarea cho Description

        self.price_label = (By.XPATH, "//label[@for='field-:r1f:']")  # Label cho Price
        self.price_input = (By.XPATH, "//input[@name='price']")  # Trường input cho Price

        self.photos_label = (By.XPATH, "//label[@for='field-:r1g:']")  # Label cho Photos
        self.add_photo_button = (By.XPATH, "//button[text()='Add a Photo']")  # Nút "Add a Photo"
        self.photo_input = (By.XPATH, "//input[@name='photos.0']")

        self.update_product_button = (By.XPATH, "//button[@type='submit']")  # Nút "Add Product"
        self.message_update_product_successfully = (By.XPATH, "//span[text()='The product successfully updates']")
    ...
    def click_update_product(self):
        ...
        self.driver.find_element(*self.update_product_button).click()

    def is_success_message_appeared(self):
        try:
            # Chờ đợi tối đa 10 giây cho thông báo "Add product successfully" xuất hiện
            WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(self.message_update_product_successfully)
            )
